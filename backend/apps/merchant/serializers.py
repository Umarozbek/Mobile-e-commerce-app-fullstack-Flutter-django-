...
from .models import Bonus, LoyaltyCard, Referral, LoyaltyPendingBonus, ReferralSettings
...
class OrderListSerializer(serializers.ModelSerializer):
    ...
    # Bank karta ma'lumotlari (BankCardModel dan olinadi)
    ...
    # Yetkazib berish narxi (Service modelidan Decimal qiymatni oladi)
    delivery_fee = serializers.ReadOnlyField(source='delivery_fee.delivery_fee')
    ...
    class Meta:
        ...
        fields = [
            ...
            'address',
            'total_amount',
            'delivery_fee',  # Xizmat narxi (summa ko'rinishida)
            'bank_card_number',  # Karta raqami
            'bank_card_holder',  # Karta egasi
            'bonus_amount',
            'loyalty_payment',
            'comment',
            'products_details',
    ...
    def get_products_details(self, obj):
        ...
        for item in obj.orderitem.all():
            ...
            if p:
                ...
                result.append({
                    "id": p.id,
                    "order_item_id": item.id,  # O'chirish uchun kerak bo'lishi mumkin
                    "names": names,
...
class LoyaltyCardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LoyaltyCard
        fields = [
            'id', 'profile', 'full_name', 'current_balance',
            'cycle_start', 'cycle_end', 'cycle_days',
            'cycle_number', 'created_at', 'updated_at',
        ]
        read_only_fields = ['id', 'full_name', 'created_at', 'updated_at']

    def to_representation(self, instance):
        # 1. Avval bazadagi ma'lumotni tekshirib, kerak bo'lsa yangilaymiz
        try:
            # Modelda biz yozgan check_and_reset funksiyasini chaqiramiz
            instance.check_and_reset()
        except Exception as e:
            # Agar biror xato bo'lsa (masalan bazada muammo),
            # dastur to'xtab qolmasligi uchun log qilamiz
            print(f"Loyalty reset error: {e}")

        # 2. Keyin standart JSON formatini qaytaramiz
...
class ReferralHistorySerializer(serializers.ModelSerializer):
    ...
    # ESLATMA: ilgari bu yerda qattiq kodlangan 5000 qiymati bo'lgan, lekin
    # ReferralSettings.bonus_amount admin tomonidan sozlanadigan bo'lgach,
    # API haqiqatda berilgan summani emas, doim 5000ni ko'rsatib xato ma'lumot
    # berardi (topilgan xato). Endi joriy sozlangan qiymatni qaytaradi.
    ...
    def get_bonus_amount(self, obj):
        return ReferralSettings.get_solo().bonus_amount
...
class B2BStatusResponseSerializer(serializers.Serializer):
    status = serializers.CharField(help_text="PENDING, WAITING, yoki B2B_MEMBER")
