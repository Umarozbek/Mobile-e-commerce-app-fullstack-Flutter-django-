...
class CategoryListAPIView(generics.ListAPIView):
    queryset = Category.objects.all().order_by("-is_top", "-pk")
...
class GoodFilter(FilterSet):
    # Mavjud kategoriya filtri
    category = django_filters.NumberFilter(field_name="category")

    # 1. Active filtri (ProductItem ichidagi active maydoniga ulanadi)
    # Swaggerda true/false tanlash imkoni chiqadi
    active = django_filters.BooleanFilter(field_name="product__active")

    # 2. Discount filtri (Chegirmadagi mahsulotlarni topish uchun)
    # is_discount=true yuborilsa, method ishga tushadi
    is_discount = django_filters.BooleanFilter(method='filter_is_discount')

    class Meta:
        model = Good
        # Qolgan standart fieldlar
        fields = ["product", "product__product_type"]

    def filter_is_discount(self, queryset, name, value):
        if value:
            # product ichidagi discount_price 0 dan katta bo'lsa - demak chegirma bor
            return queryset.filter(product__discount_price__gt=0)
        return queryset
...
class GoodListAPIView(ProductOptimizationMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        return (
            self.get_optimized_queryset(Good)
            .filter(product__main=True)
            .select_related("product")
            .prefetch_related("product__images")  # 👈 SHUNI QO‘SH
        )
...
class GoodAllListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # .filter(product__active=True) — mana shu qism nofaol mahsulotlarni qirqib tashlaydi
        queryset = Good.objects.filter(
            product__active=True,  # Faqat aktiv mahsulotlar
            product__main=True  # Faqat asosiy mahsulotlar (variantlar ro'yxatda takrorlanmasligi uchun)
