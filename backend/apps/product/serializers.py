...
class ProductVariantFullSerializer(serializers.ModelSerializer):
    ...
    def get_prices(self, obj):
        # 1. Mahsulot obyektini aniqlaymiz
        p = obj.product if hasattr(obj, 'product') else obj

        request = self.context.get('request')
        user = request.user if request else None

        # 2. Hamma uchun ko'rinadigan asosiy narxlar (Chakana)
        prices = {
            "price": float(p.new_price) if p.new_price else 0,
            "old_price": float(p.old_price) if p.old_price else 0,
            "discount_percent": p.sale,
            "min_wholesale_quantity": p.min_wholesale_quantity,
        }

        # 3. B2B va Optom narxlar uchun XAVFSIZ tekshiruv
        # Avval user login qilganmi (is_authenticated) shuni ko'ramiz
        if user and user.is_authenticated:
            # Login qilgan bo'lsa, endi uning maydonlarini getattr orqali xavfsiz olamiz
            ...
            if is_approved and is_wholesaler:
                ...
                prices["wholesale_price"] = float(p.wholesale_price) if p.wholesale_price else 0
            else:
                prices["b2b_price"] = None
                prices["wholesale_price"] = None
        else:
            # Agar foydalanuvchi mehmon bo'lsa (AnonymousUser), narxlarni yashiramiz
            prices["b2b_price"] = None
            prices["wholesale_price"] = None
