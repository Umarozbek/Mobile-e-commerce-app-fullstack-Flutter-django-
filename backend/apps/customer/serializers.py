...
from .models import Favorite, Location, News, Profile, ViewedNews, Banner, B2BApplication, DeviceToken
...
class DeviceTokenRegisterSerializer(serializers.Serializer):
    """POST /api/customer/device-token/register/ uchun."""
    token = serializers.CharField(max_length=255, allow_blank=False, trim_whitespace=True)
    platform = serializers.ChoiceField(
        choices=DeviceToken.PLATFORM_CHOICES, required=False, default=DeviceToken.PLATFORM_ANDROID
    )

    def validate_token(self, value):
        value = value.strip()
        if len(value) < 10:
            raise serializers.ValidationError("Token juda qisqa - yaroqsiz FCM token.")
        return value


class DeviceTokenRemoveSerializer(serializers.Serializer):
    """DELETE /api/customer/device-token/ uchun."""
    token = serializers.CharField(max_length=255, allow_blank=False, trim_whitespace=True)
...
class NotificationPublicSerializer(serializers.ModelSerializer):
    ...
    NewsSerializer bilan bir xil model (News), lekin faqat foydalanuvchiga
    tegishli maydonlarni qaytaradi (admin-only 'active' maydoni chiqarilmaydi -
    chunki bu yerga faqat active=True va joriy sana oralig'idagilar keladi).
    """
    class Meta:
        model = News
        fields = [
            "id", "title", "description", "image", "link", "type",
            "priority", "start_date", "end_date", "created", "modified",
        ]

    def to_representation(self, instance):
        rep = super().to_representation(instance)
        # ESLATMA: NewsSerializer'dagi kabi settings.DOMAIN_NAME + url ishlatilmadi -
        # DOMAIN_NAME odatda sxemasiz (masalan "localhost"), shuning uchun natija
        # to'liq (accessible) URL bo'lmay qolardi. Buning o'rniga product/good
        # rasmlarida ishlatiladigan request.build_absolute_uri() patterni
        # qo'llanildi - u so'rovning haqiqiy sxema+host'idan to'g'ri URL quradi.
...
class TokenToJWTSerializer(serializers.Serializer):
