...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # 2. Agar token kelgan bo'lsa, uni profilga saqlaymiz (ESKI, moslik uchun)
            if device_token:
                profile.device_token = device_token
                profile.save(update_fields=["device_token"])

                # 3. YANGI: xuddi shu tokenni DeviceToken jadvaliga ham ro'yxatdan
                # o'tkazamiz/yangilaymiz - push yuborish endi shu yerdan o'qiydi.
                # Bu login orqali eski Profile.device_token'li foydalanuvchilarni
                # ham avtomatik "quvib yetadi" (backfill) - alohida migratsiya
                # kutish shart emas.
...
class DeviceTokenRegisterAPIView(APIView):
    """
    POST /api/customer/device-token/register/

    Joriy (JWT bilan autentifikatsiya qilingan) foydalanuvchining FCM
    qurilma tokenini ro'yxatdan o'tkazadi. Bitta foydalanuvchi bir nechta
    qurilmaga ega bo'lishi mumkin (masalan telefon + planshet) - har biri
    alohida DeviceToken yozuvi sifatida saqlanadi.

    Token boshqa profilga tegishli bo'lib turgan bo'lsa (masalan shu
    qurilmada boshqa foydalanuvchi oldin kirgan bo'lsa), u joriy profilga
    qayta biriktiriladi va qayta faollashtiriladi.
    """
    ...
    @swagger_auto_schema(
        ...
        operation_summary="FCM qurilma tokenini ro'yxatdan o'tkazish",
        request_body=DeviceTokenRegisterSerializer,
    )
    def post(self, request):
        ...
        return Response({
            "message": "Qurilma ro'yxatdan o'tkazildi" if created else "Qurilma yangilandi",
            "created": created,
            "id": device.id,
...
@extend_schema(tags=["Push Notifications"])
class DeviceTokenRemoveAPIView(APIView):
    """
    DELETE /api/customer/device-token/

    Joriy foydalanuvchining shu qurilma tokenini FAOLSIZLANTIRADI (chiqish/
    logout paytida chaqiriladi). Yozuv o'chirilmaydi - shunchaki push
    yuborish ro'yxatidan chiqariladi (is_active=False), tarixiy audit uchun
    saqlanib qoladi.
    """
    permission_classes = [IsAuthenticated]
    serializer_class = DeviceTokenRemoveSerializer

    @swagger_auto_schema(
        tags=["Push Notifications"],
        operation_summary="FCM qurilma tokenini faolsizlantirish",
        request_body=DeviceTokenRemoveSerializer,
    )
    def delete(self, request):
        ...
        if not updated:
            return Response(
                {"message": "Token topilmadi"}, status=status.HTTP_404_NOT_FOUND
            )
        return Response({"message": "Qurilma faolsizlantirildi"}, status=status.HTTP_200_OK)
...
class NotificationListAPIView(ListAPIView):
    """
    Mobil ilova (Flutter) uchun ochiq bildirishnomalar ro'yxati.

    Faqat active=True va joriy sana oralig'idagi (start_date/end_date)
    yozuvlarni qaytaradi, priority DESC keyin created DESC bo'yicha
    tartiblangan. Ixtiyoriy `type` query parametri bilan filtrlash mumkin
    (masalan: ?type=SALE).
    """
            ...
            with transaction.atomic():
                ...
                if profile_created and referral_code:
                    try:
                        ...
                        if referrer != profile:
                            # Tizim yoqilgan bo'lsa - avtomatik bonus ('rewarded'),
                            # aks holda 'pending' (admin keyinchalik qo'lda tasdiqlaydi)
                            auto_active = ReferralSettings.get_solo().referral_system_active
                            Referral.objects.create(
                                referrer=referrer,
                                referee=profile,
                                status='rewarded' if auto_active else 'pending'
                            )
                            referral_bonus_given = auto_active
                    except Profile.DoesNotExist:
...
def send_otp_sms(phone_number, otp):
    ...
    try:
        ...
        data = {
            "To": phone_number,
            "From": settings.TWILIO_PHONE_NUMBER,  # 🔥 MessagingService o‘rniga
            "Body": f"Code: {otp}"
        }
...
class TokenToJWTView(APIView):
    permission_classes = [AllowAny]
    authentication_classes = []
    serializer_class = TokenToJWTSerializer  # <- shu qo'shiladi

    @swagger_auto_schema(
        operation_summary="Old token orqali JWT olish",
        request_body=TokenToJWTSerializer,
