...
class GoodProductItemForm(forms.ModelForm):
    # ProductItem uchun kerakli maydonlar
    category = forms.ModelChoiceField(queryset=Category.objects.filter(main_type="f"), label="Kategoriyasi")
    new_price = forms.DecimalField(initial=0)
    discount_price = forms.DecimalField(required=False, initial=0, label="Eski narxi")
    b2b_price = forms.DecimalField(required=False, initial=0)
    available_quantity = forms.IntegerField(initial=0)
    weight = forms.FloatField(required=False, initial=1.0)
    measure = forms.ChoiceField(choices=ProductItem.CHOICES)

    # 4 tilda Nomlar
    name_uz = forms.CharField(max_length=255, required=True, label="Nomi UZ")
    name_ru = forms.CharField(max_length=255, required=False, label="Nomi RU")
    name_en = forms.CharField(max_length=255, required=False, label="Nomi EN")
    name_ko = forms.CharField(max_length=255, required=False, label="Nomi KO")

    # 4 tilda Tavsiflar
    desc_uz = forms.CharField(widget=forms.Textarea(attrs={'rows': 3}), required=False)
    desc_ru = forms.CharField(widget=forms.Textarea(attrs={'rows': 3}), required=False)
    desc_en = forms.CharField(widget=forms.Textarea(attrs={'rows': 3}), required=False)
    desc_ko = forms.CharField(widget=forms.Textarea(attrs={'rows': 3}), required=False)
    ...
    class Meta:
        model = Good
        # Good modelida bor maydonlar
        fields = ["ingredients", "expire_date"]
    ...
    def save(self, commit=True):
        # 1. Good ob'ektini yaratamiz
        good = super().save(commit=False)
        good.name = self.cleaned_data.get("name_uz")  # Asosiy nomga UZ ni beramiz
        good.category = self.cleaned_data.get("category")

        # 2. ProductItem yaratamiz
        # desc maydoniga uzb tavsifni beramiz (modelda desc_uz bo'lmasa)
        product_item = ProductItem.objects.create(
            name=self.cleaned_data.get("name_uz"),
            desc=self.cleaned_data.get("desc_uz", ""),
            measure=self.cleaned_data["measure"],
            new_price=self.cleaned_data["new_price"],
            old_price=self.cleaned_data.get("discount_price", 0),
            b2b_price=self.cleaned_data.get("b2b_price", 0),
            available_quantity=self.cleaned_data["available_quantity"],
            weight=self.cleaned_data.get("weight", 1.0),
            active=self.cleaned_data.get("active", True),
            main=self.cleaned_data.get("main", False),
        )
...
class GoodEditForm(forms.ModelForm):
    # --- ProductItem modelidan keladigan maydonlar ---
    category = forms.ModelChoiceField(
    ...
    b2b_price = forms.DecimalField(
        ...
        widget=forms.NumberInput(attrs={"class": "sherah-wc__form-input", "placeholder": "B2B narxi"})
    )
    available_quantity = forms.IntegerField(
        widget=forms.NumberInput(attrs={"class": "sherah-wc__form-input"})
    )
    ...
    # --- ProductItem ichidagi Tavsiflar (4 tilda) ---
    desc_uz = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "sherah-wc__form-input", "rows": "3", "placeholder": "Tavsif UZ"})
    )
    desc_ru = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "sherah-wc__form-input", "rows": "3", "placeholder": "Tavsif RU"})
    )
    desc_en = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "sherah-wc__form-input", "rows": "3", "placeholder": "Tavsif EN"})
    )
    desc_ko = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={"class": "sherah-wc__form-input", "rows": "3", "placeholder": "Tavsif KO"})
    )
    ...
    class Meta:
        model = Good
        fields = ["name_uz", "name_ru", "name_en", "name_ko", "expire_date", "category", "ingredients"]
        widgets = {
            "name_uz": forms.TextInput(attrs={"class": "sherah-wc__form-input"}),
            "name_ru": forms.TextInput(attrs={"class": "sherah-wc__form-input"}),
            "name_en": forms.TextInput(attrs={"class": "sherah-wc__form-input"}),
            "name_ko": forms.TextInput(attrs={"class": "sherah-wc__form-input"}),
            "expire_date": forms.DateInput(attrs={"class": "sherah-wc__form-input", "type": "date"}),
            "ingredients": forms.TextInput(attrs={"class": "sherah-wc__form-input"}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.product:
            ...
            self.fields["b2b_price"].initial = p.b2b_price
            self.fields["available_quantity"].initial = p.available_quantity
            self.fields["weight"].initial = p.weight
            self.fields["measure"].initial = p.measure
            self.fields["active"].initial = p.active
            self.fields["desc_uz"].initial = p.desc_uz
            self.fields["desc_ru"].initial = p.desc_ru
            self.fields["desc_en"].initial = p.desc_en
            self.fields["desc_ko"].initial = p.desc_ko
    ...
    def save(self, commit=True):
        ...
        product_item.b2b_price = self.cleaned_data["b2b_price"]
        ...
        product_item.desc_uz = self.cleaned_data.get("desc_uz", "")
        product_item.desc_ru = self.cleaned_data.get("desc_ru", "")
        product_item.desc_en = self.cleaned_data.get("desc_en", "")
        product_item.desc_ko = self.cleaned_data.get("desc_ko", "")

        # --- RASMLARNI O'CHIRISH QISMI ---
        # HTML'dagi hidden input'dan o'chirilishi kerak bo'lgan ID'larni olamiz
        removed_images_ids = self.data.get('removed_images')
        if removed_images_ids:
            # ID'larni listga aylantiramiz va bazadan o'chiramiz
            id_list = [id_val for id_val in removed_images_ids.split(',') if id_val.isdigit()]
            Image.objects.filter(id__in=id_list, product=product_item).delete()

        if commit:
            product_item.save()
            good.save()

            # Yangi rasm yuklangan bo'lsa saqlash
            new_img = self.cleaned_data.get("image")
            if new_img:
                Image.objects.create(
                    image=new_img,
                    name=f"edit_{good.name_uz}",
...
class NewsForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "link": forms.URLInput(attrs={"class": "form-control", "placeholder": "https:// yoki ichki route"}),
    ...
    def clean(self):
        ...
        if start_date and end_date and end_date < start_date:
            raise forms.ValidationError("Tugash sanasi boshlanish sanasidan oldin bo'lishi mumkin emas.")
        return cleaned_data
...
class CategoryEditForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance:
            ...
            self.fields["is_top"].initial = self.instance.is_top
...
class BankCardForm(forms.ModelForm):
    class Meta:
        model = BankCardModel
        fields = ['title', 'card_holder']
        widgets = {
            'title': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Karta raqami (faqat raqamlar)'}),
            'card_holder': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Karta egasining ismi'}),
        }
        labels = {
            'title': 'Karta raqami',
...
class ServiceForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['delivery_fee']
        widgets = {
            'delivery_fee': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Masalan: 5000'}),
        }
        labels = {
...
class OrderBonusTierForm(forms.ModelForm):
    class Meta:
        model = OrderBonusTier
        fields = ['min_amount', 'max_amount', 'bonus_amount', 'active']
        widgets = {
            'min_amount': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Masalan: 0'}),
            'max_amount': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Bo\'sh = yuqori chegara yo\'q'}),
            'bonus_amount': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Masalan: 3000'}),
            'active': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
        }
        labels = {
    ...
    def clean(self):
        ...
        if min_amount is not None and max_amount is not None and max_amount < min_amount:
            raise forms.ValidationError("Maksimal summa minimal summadan kichik bo'lishi mumkin emas.")
        return cleaned_data


class TelegramSettingsForm(forms.ModelForm):
    """bot_token maydoni HECH QACHON joriy qiymat bilan to'ldirilmaydi
    (maskalangan/bo'sh chiqadi) va bo'sh saqlansa eski qiymat o'zgarmaydi -
    save() metodida qayta ishlanadi."""
    bot_token = forms.CharField(
        required=False,
        widget=forms.PasswordInput(render_value=False, attrs={
            'class': 'form-control', 'placeholder': "O'zgartirish uchun yangi tokenni kiriting",
            'autocomplete': 'new-password',
        }),
        label="Bot Token",
        help_text="Xavfsizlik uchun hech qachon to'liq ko'rsatilmaydi. Bo'sh qoldirsangiz - eski token saqlanadi.",
    )
    ...
    def save(self, commit=True):
        ...
        new_token = self.cleaned_data.get('bot_token')
        if not new_token:
            # Bo'sh saqlansa - bazadagi eski tokenni tegib turmaymiz
...
class PushComposeForm(forms.Form):
    """Admin broadcast push xabarnomasini yozish uchun.

    ESLATMA: yuborilgan xabar endi News (Notification) sifatida ham
    saqlanadi (bog'liq emas emas - apps/dashboard/users.py:push_compose_view
    qarang) - shu sababli maydonlar News modeliga mos: title/description/
    image/link/type/priority.
    """
    title = forms.CharField(
        max_length=255, required=True,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "Sarlavha"}),
        label="Sarlavha",
    )
    body = forms.CharField(
        required=True,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 4, 'placeholder': "Xabar matni"}),
        label="Matn",
    )
    image = forms.ImageField(
        required=False,
    ...
    type = forms.ChoiceField(
        choices=News.TYPE_CHOICES, required=False, initial=News.TYPE_GENERAL,
        widget=forms.Select(attrs={'class': 'form-control'}),
        label="Turi",
    )
    ...
    link = forms.CharField(
        ...
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': "/sale yoki https://... (ixtiyoriy)"}),
